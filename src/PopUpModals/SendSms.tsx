...
const SendSms = () => {
  const [showPopoutMessage, setShowPopoutMessage] = useState(false);

  const handleSendSms = () => {
    setShowPopoutMessage(true);
  };

  const handleClosePopoutMessage = () => {
    setShowPopoutMessage(false);
  };
  ...
  return (
    <div className='send-sms-container'>
      <div className='send-sms-topic-container'>
        ...
        <div className='send-sms-topic'>Send an SMS</div>
      ...
      <div className='input-box-container'>
         <textarea className='input-box' placeholder='Type your message here'>
          <div className='input-box-text'></div> 
            
        </textarea> 
        <div className='text-count'>0/240</div> 
      </div>

      <button className='send-sms-button-container' onClick={handleSendSms}>
        <div className='send-sms-button-title'>Send SMS</div>
      ...
      {showPopoutMessage && <SmsSentSuccess onClose={handleClosePopoutMessage} />}
    </div>
  );
};
